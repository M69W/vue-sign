    ...
    <ul class="clearfix">
      <li class="Uploadfiles"
        ...
        <span v-if="path!='addukeysign'">
          <div class="filename"
               v-if="item.certificateId<=5">{{item.certificateName}}<a href='../../static/image/企业电子印章申请表及用户责任书.docx'
               class='download'
               v-if="item.certificateId==5">下载申请书模板</a></div>
          <span class="close"
                v-if="item.certificateId>5"
                @click="close(key)"></span>
          <el-input type="text"
                    placeholder="请输入附件名称"
                    v-if="item.certificateId>5"
        ...
        <span v-if="path=='addukeysign'">
          <div class="filename"
               v-if="item.certificateId<=2">{{item.certificateName}}<a href='../../static/image/个人电子印章申请表及用户责任书.docx'
               class='download'
               v-if="item.certificateId==2">下载申请书模板</a></div>
          <span class="close"
                v-if="item.certificateId>2"
                @click="close(key)"></span>
          <el-input type="text"
                    placeholder="请输入附件名称"
                    v-if="item.certificateId>2"
    ...
    <el-dialog :visible.sync="dialogModel"
      ...
      <div class="clearposition">
        <div class="arrLeft arrow"
             @click="changePicture('0')"><i></i></div>
        <div class="arrRight arrow"
             @click="changePicture('1')"><i></i></div>
        <div class="certname">{{certName}} <i @click="handleClose"></i> </div>
  ...
  methods: {
    ...
    cbSuccess(data) {
      if (this.successCheckCode(data)) {
        ...
        this.certInfoList[ind].certificatePath = data.data.certificateFilePath;
      }
    },
    cbError() {
      var event = this.eve;
...
.certInfo {
  ...
  li.Uploadfiles {
    ...
    .upinfo {
      ...
      cursor: pointer;
      .bg {
        background: #ffffff url(../../static/image/icon_add_hover.svg) 50% 40px
          no-repeat;
        width: 138px;
        height: 138px;
