  ...
  <div class="ukeyrenew">
    <div class="orderpage-topbar clearfix">
      <div class="orderpage-tit"
           v-if="typeVal == '01'">Ukey签名续期</div>
      <div class="orderpage-tit"
           v-if="typeVal == '02'">Ukey印章续期</div>
    ...
    <div class="orderpage-cont">
      ...
      <div class="payType-list clearfix">
        <el-radio-group v-model="payTypeVal"
                        size="small">
          <el-radio :label="1"
                    :class="payTypeVal == 1 ? 'actived' : ''">
            <img src="../../../../static/image/wechatpay.png" />
            <i v-if="payTypeVal == 1"></i>
          </el-radio>
          <el-radio :label="2"
                    :class="payTypeVal == 2 ? 'actived' : ''">
            <img src="../../../../static/image/alipay.png" />
            <i v-if="payTypeVal == 2"></i>
          </el-radio>
...
import common, { isEmail } from "../common/index";
...
export default {
  ...
  computed: {
    compSealValidDateEnd() {
      let year = Number(this.sealValidDateEnd.substr(0, 4));
      return (
        year + Number(this.yearNum) + this.sealValidDateEnd.substr(4)
      );
    }
  },
  watch: {
  ...
  async mounted() {
    ...
    // this.yearNum = this.$route.query.yearNum;
    // this.price = parseInt(this.yearNum) * 100;
    if (this.$route.path == "/ukeyrenewpay") {
      //ukey签名续期申请流程
      this.typeVal = "01";
      this.productName = "UKEY签名续期";
    } else if (this.$route.path == "/ukeysealrenewpay") {
      //ukey印章续期申请流程
      this.typeVal = "02";
      this.productName = "UKEY印章续期";
    }
  ...
  methods: {
    toPay() {
      if (
        ...
        this.invoiceType == 3 &&
        (!this.invoiceTitle || !this.buyerEmail)
      ) {
        return this.$message.error("请输入开票信息");
    ...
    gotoList() {
      this.changeOrderStatus(this.orderNo).then(res => {
    ...
    getOrderConfirmData() {
      return this.getRenewOrderConfirm(this.orderNo, this.pageNum, 100).then(res => {
        if (this.successCheckCode(res)) {
          ...
          this.sealValidDateStart =
            res.data.sealList[0].sealValidDateStart;
          this.sealValidDateEnd =
            res.data.sealList[0].sealValidDateEnd;
        }
      });
    },
    getOrderDetail() {
      return this.orderDetailEnterprise(this.orderNo).then(res => {
        if (this.successCheckCode(res)) {
          ...
          this.yearNum = res.data.esealList[0].renewalTime;
          // this.price = this.totalPrice; //Number(this.yearNum) * 100;
          this.price = res.data.esealList[0].price;
          this.pictureCode = res.data.esealList[0].pictureCode;
    ...
    getAliPayCodeData() {
      var data = {
        ...
        tradeType: "03", // 01：ukey订单，02：套餐订单 03 续期
...
.payType-list .ukeycost.actived {
  border: 1px solid #02adff;
}
.payType-list .ukeycost.actived i {
  display: block;
  position: absolute;
  right: 0;
  top: 0;
  width: 70px;
  height: 70px;
  background: url(../../../../static/image/icon-selected-big.png) no-repeat;
}
</style>
