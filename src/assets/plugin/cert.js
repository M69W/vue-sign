...
var certUtil = (function() {
    ...
    var NETCAPKI_UIFLAG_DEFAULT_UI = 1; //不显示UI
    ...
    */
    ...
    function getCertFromDeviceByCrypto(filter, deviceType, deviceFlag, UI, tips) {
        ...
        try {
            ...
            //默认不弹出选择证书的选择框
            UI=NETCAPKI_UIFLAG_DEFAULT_UI;
            cert = deviceSet.FindCertificate(filter, UI);
            if (cert == null) {
                console.log("不弹窗未找到证书，设备数：" + deviceSet.count)
                if(deviceSet.count <= 0){
                    alert("没有插入设备");
                    return null;
                }
                //如果尝试查找只有一张证书的情况失败时，则弹窗给用户选择（可能不止一个Key或者Key中不止一张证书）。
                cert = deviceSet.FindCertificate(filter, NETCAPKI_UIFLAG_ALWAYS_UI);
            }
            utils = null;
            return cert;
        } catch (e) {
