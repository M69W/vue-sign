var certUtil = require('./cert').default;
//依赖cert.js
var netca = (function() {
    /**
    ...
    function delCert(deleteCertList) {
        ...
        // }];
        var result = [];
        for (var i = 0; i < deleteCertList.length; i++) {
            result.push({
                "issuer": deleteCertList[i].issure,
                "sn": deleteCertList[i].certSN
            })
        }
        console.log(result)
        var query = {
            deleteCertList: result
        };
        //query 填null时显示所有证书
        var isSuccess = certUtil.delCertInDevice("-1", "-1", query); //query设置为空显示所有证书
        return isSuccess;
