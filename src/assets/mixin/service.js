...
import { checkBlobFileResponseUtils, getBroswer, isIE } from "@/utils";
...
let signUrl =
    "/compact/file/page?fileCode=a8c20fb7-2dc3-4341-8cb5-dcbe4da41a68&pageNo=1";
const UKeyDriveName = "";
const UKeyDriveSrc = "";
...
//ali支付sessionStorage失效问题
if (!sessionStorage.getItem("token")) {
    sessionStorage.setItem("token", $.cookie("token"));
    sessionStorage.setItem("sessionToken", $.cookie("sessionToken"));
}
...
var ajaxCall = function (setting, type) {
    ...
    let reg = /([\u4e00-\u9fa5])/g;
    let url = setting.url;
    let resUrl = url.replace(reg, function (val) {
    ...
    if (
        ...
        // if (Object.keys(setting.headers).length > 0) {
        // }
        if (typeof setting.contentType === "undefined" || setting.contentType) {
            // formData传参不需要stringify
...
const info = {
    ...
    internetError: "网络不稳定，请稍后重试",
    sealError: "章模读取出错了，请联系客服",
};
export default {
    data() {
        return {
            realNameImgUrl: "/api/user/user-real-name-auth/file/preview",
        };
    },
    methods: {
        successCheckCode(ret) {
            if (ret.code === 401) {
                ...
                if (window.location.href.indexOf('iyinreadersign') > -1) {
        ...
        ajaxCall(setting, type) {
            ...
            xhr.fail((jqxhr, textStatus) => {
                if (textStatus != "abort") {
                    this.$message({
                        message:
                            JSON.parse(jqxhr.responseText).msg ||
                            info.defaultErrorMsg,
                        type: "warning",
                        duration: 1500,
                    });
                }
        ...
        ajaxUKeyCall(setting, type) {
            ...
            // div.innerText = 'err'+  JSON.stringify(xhr);
            ...
            xhr.fail((jqxhr, textStatus) => {
                if (textStatus == "error") {
                    ...
                    if (broswer.broswer == "IE") {
                        console.log(broswer);
                        if (["9.0", "8.0"].includes(broswer.version)) {
                            this.$message({
                                message: "请安装最新IE浏览器(IE10以上)！",
                                type: "error",
                                duration: 1500,
                            });
                            return;
            ...
            xhr.done((data, textStatus, jqxhr) => {
                ...
                if (setting.url == ctx + "getVersion" && datas.ret != 0) {
                    this.$updateUKeyDrive();
                }
                if (setting.url != ctx + "getVersion" && datas.ret != 0) {
                    this.$message({
                        message: info.sealError,
                        type: "error",
                        duration: 1500,
                    });
                }
        ...
        addTemplate(data) {
            return this.ajaxCall(
                {
                    url: domain + api + "/signSysTemplate/addTemplate",
                    data: data,
                },
                "post"
            );
        ...
        getPicSize(fileCode) {
            return this.ajaxCall(
                {
                    url:
                        domain + api + "/document/fetch/getPicSize/" + fileCode,
                },
                "get"
            );
        },
        ...
        //企业注册用户名是否已使用
        checkedEnterPriseName(name) {
            return this.ajaxCall({
                url: `${domain}${api}/user/enterprise/register/enable?enterpriseName=${name}`,
            });
        },
        //邮箱验证码
        emailCode(data) {
        ...
        // 检测用户是否可以作为指定的支付用户
        filterEligibleUs(data) {
            return this.ajaxCall(
                {
                    url: domain + api + "/compact/filterEligibleUser",
                    data: data,
                },
                "post"
            );
            ...
            return this.ajaxCall(
                {
                    url:
                        domain +
                        api +
                        "/compact/sign/field/info/detail/" +
                        data,
                },
                "get"
            );
        ...
        editCompactSec(data) {
            return this.ajaxCall(
                {
                    url:
                        domain + api + "/compact/signatory/info/detail/" + data,
                },
                "get"
            );
        },
        ...
        //批量删除合同
        deleteCompactBatch(data) {
            return this.ajaxCall(
                { url: `${domain}${api}/compact/info/batch`, data: data },
                "delete"
            );
        },
        //订单列表
        ...
        dowloadFileAll(data) {
            let obj = {
                data: data,
                url: domain + api + "/compact/package/download/batch",
            };
            this.downloadfiles(obj);
        },
        // blob 的下载
        downloadfiles({ data, url } = {}) {
            // 下载
            let _self = this;
            var datas = {
            ...
            xhr.setRequestHeader(
                "Authorization",
                $.cookie("token") || sessionStorage.getItem("token")
            );
        ...
        deleteOrderSeal(sealId) {
            return this.ajaxCall(
                {
                    url:
                        domain + api + "/seal/enterprise/custom/info/" + sealId,
                },
                "delete"
            );
        },
        ...
        compactMore(compactId) {
            return this.ajaxCall(
                {
                    url:
                        domain + api + "/compact/info/more/detail/" + compactId,
                },
                "get"
            );
        },
        ...
        checkSignPwd(data) {
            return this.ajaxCall(
                {
                    url: domain + api + "/compact/check/signPassword",
                    data: data,
                },
                "post"
            );
        ...
        getSealRenewInfo(data) {
            return this.ajaxCall(
                {
                    url: domain + api + "/order/extension/eseal",
                    data: data,
                },
                "post"
            );
        ...
        // --------------------------------------
        // 存证报告下载
        // apiDownloadCompactReport(data) {
        //     return this.ajaxCall(
        //         {
        //             url: domain + api + '/compact/report/download',
        //             data
        //         },
        //         'post'
        //     )
        ...
        getLoginSms(phone) {
            return this.ajaxCall(
                {
                    url: domain + api + "/user/loginsms/uc/" + phone,
                },
                "get"
            );
        },
        ...
        getSecurityCode() {
            return this.ajaxCall(
                {
                    url: domain + api + "/captcha/getCode",
                },
                "get"
            );
        },
        // 检验安全验证码
        checkSecurityCode(data) {
            return this.ajaxCall(
                {
                    url: domain + api + "/captcha/checkCode",
                    data: data,
                },
                "get"
            );
            ...
            return this.ajaxCall(
                {
                    url:
                        domain +
                        api +
                        "/background/getEnableList?groupId=" +
                        type,
                },
                "get"
            );
        ...
        //范本查询分页列表
        getTempModelList(data) {
            return this.ajaxCall(
                {
                    url: domain + api + "/compact/template/example/list/page",
                    data: data,
                },
                "post"
            );
        ...
        //模板范本分类数量统计
        getTempModelType(data) {
            return this.ajaxCall(
                {
                    url: domain + api + "/compact/template/example/type/count",
                    data: data,
                },
                "get"
            );
        ...
        tempModelDetail(id) {
            return this.ajaxCall(
                {
                    url:
                        domain + api + "/compact/template/example/detail/" + id,
                },
                "get"
            );
        },
            ...
            return this.ajaxCall(
                {
                    url:
                        domain +
                        api +
                        "/compact/template/example/download/url/" +
                        id,
                },
                "get"
            );
        ...
        getPrintHtml(id) {
            return this.ajaxCall(
                {
                    url: domain + api + "/compact/html/" + id,
                },
                "get"
            );
        },
        ...
        getOff() {
            return this.ajaxCall(
                {
                    url: domain + api + "/user/logoff/normal",
                },
                "post"
            );
        },
        ...
        getUkeyPrice(data) {
            return this.ajaxCall(
                {
                    url: domain + api + "/order/getUkeyPrice",
                    data: data,
                },
                "get"
            );
        ...
        signSysList(data) {
            return this.ajaxCall(
                {
                    url: domain + api + "/contract/sign/log/signSystem/list",
                    data: data,
                },
                "get"
            );
         ...
         * 公司名称&社会信用代码
         */
        checkedCreditCode(data) {
            return this.ajaxCall(
                {
                    url:
                        domain +
                        api +
                        `/transferInfo/checkCreditCode${this.parseParams(
                            data
                        )}`,
                },
                "post"
        ...
        checkedNameAndIdCard(data) {
            return this.ajaxCall(
                {
                    url:
                        domain +
                        api +
                        `/transferInfo/checkNameAndID${this.parseParams(data)}`,
                },
                "post"
            );
        },
        saveVarifyCompanyInfo(data) {
            return this.ajaxCall(
                {
                    url: domain + api + `/transferInfo/save`,
                    data,
                },
                "post"
            );
        ...
        //检测当前对公转账认证企业是否实名过
        checkCreditCodeAndName(data) {
            return this.ajaxCall(
                {
                    url: domain + api + `/transferInfo/checkCreditCodeAndName`,
                    data,
                },
                "post"
            );
        },
        getPublicTransferByUserId(userId) {
            return this.ajaxCall({
                url: domain + api + `/transferInfo/getPublicTransferByUserId`,
            });
        },
        /**
         * 获取银行列表
         * @param {string} bankName //银行名字
         * @param {string} bankType //类型
         */
        getBanksByName(bankName, bankType) {
            return this.ajaxCall(
                {
                    url: domain + api + `/transferInfo/getBanksByName`,
                    data: {
                        bankName,
                        bankType,
                    },
        ...
        reSubmitPubTransfer() {
            return this.ajaxCall({
                url: domain + api + `/transferInfo/submitPubTranAgain`,
            });
        },
        //对公转账重验金额
        reVerifyMoney(id) {
            return this.ajaxCall(
                {
                    url: domain + api + `/transferInfo/checkMoneyAgain/${id}`,
                },
                "post"
            );
        },
        ...
        saveAuditInfo(data) {
            // $.ajax({
            //     url: domain + api + `/user/user-real-name-auth/saveAuditInfo`,
            //     type: "POST",
            //     cache: false,
            //     data: data,
            //     processData: false,
            ...
            return this.ajaxCall(
                {
                    url:
                        domain +
                        api +
                        `/user/user-real-name-auth/saveAuditInfo`,
                    headers: {},
        ...
        getAuditInfo(data) {
            return this.ajaxCall(
                {
                    url:
                        domain + api + `/user/user-real-name-auth/getAuditInfo`,
                    data,
                },
                "get"
            );
        ...
        getMsgList(data) {
            return this.ajaxCall(
                {
                    url: domain + api + `/message/info/list`,
                    // url: `${domain}${'mock'}/message/info/list`,
                    data,
                },
                "post"
            );
        ...
        getMsgDetail(id) {
            return this.ajaxCall(
                {
                    url: domain + api + `/message/getMessageNotice/${id}`,
                },
                "get"
            );
        },
        /**设置单个消息读取和删除状态
         * @data {Object} // 
         *  operateType (string, optional): 操作类型 , 1删除 .0以读
            recordIdList (Array[string], optional): 消息记录id
        */
        setSimgleMsgState(data) {
            data.userId = JSON.parse($.cookie("userinfo")).currentUserId;
            return this.ajaxCall(
                {
                    url: domain + api + `/message/updateMsgRecord`,
                    data,
                },
                "post"
            );
        },
        setMutipleMsgState(data) {
            data.userId = JSON.parse($.cookie("userinfo")).currentUserId;
            return this.ajaxCall(
                {
                    url: domain + api + `/message/updateAllMsgRecord`,
                    data,
                },
                "post"
            );
        ...
        getCouponList(data) {
            return this.ajaxCall(
                {
                    url: domain + `${api}/couponRecord/page`,
                    data,
                },
                "post"
            );
        ...
        //优惠券--可使用优惠券列表
        getEnableCouponList(data) {
            return this.ajaxCall(
                {
                    url: domain + `${api}/couponRecord/getEnableListByUserId`,
                    data: data,
                },
                "get"
            );
        ...
        getInviteList(data) {
            return this.ajaxCall(
                {
                    url: domain + `${api}/userInvite/page`,
                    data,
                },
                "post"
            );
        ...
        //获取邀请卡券
        getInviteCoupon(data) {
            return this.ajaxCall(
                {
                    url: domain + `${api}/userInvite/couponSummary`,
                    data,
                },
                "post"
            );
        ...
        getInviteMan(userId) {
            return this.ajaxCall(
                {
                    url: domain + `${api}/userInvite/getUserInfo/${userId}`,
                },
                "get"
            );
        },
        ...
        getFillFieldInfo(id) {
            return this.ajaxCall(
                {
                    url: `${domain}${api}/compact/getFillFieldInfo/${id}`,
                },
                "get"
            );
        },
        ...
        // shenzhen ///
        // 获取当前印章持有人身份证信息
        apiGetSealGrantUserInfo(data) {
            return this.ajaxCall(
                {
                    url: `${domain}${api}/elecSealGrantInfo/get`,
                    data,
                },
                "get"
            );
        ...
        // 保存印章持有人身份证信息
        apiInsertSealGrantUserInfo(data) {
            return this.ajaxCall(
                {
                    url: `${domain}${api}/elecSealGrantInfo/insert`,
                    data,
                },
                "post"
            );
        ...
        apiGetGrantWeb(data) {
            return this.ajaxCall(
                {
                    url: `${domain}${api}/elecseal/getGrantWeb`,
                    data,
                },
                "post"
            );
        ...
        // 获取授权码
        apiGetGrantCode(data) {
            return this.ajaxCall(
                {
                    url: `${domain}${api}/elecseal/getGrantCode`,
                    data,
                },
                "post"
            );
        ...
        // 根据授权码获取印章
        apiGetSealByGrantCode(data) {
            return this.ajaxCall(
                {
                    url: `${domain}${api}/elecseal/getSealByGrantCode`,
                    data,
                },
                "post"
            );
        ...
        // 根据授权码获取文档 HASH
        apiGetFileBase64(data) {
            return this.ajaxCall(
                {
                    url: `${domain}${api}/compact/file/str/${data.compactFileCode}`,
                    data,
                },
                "post"
            );
        ...
        apiGetPdfHash(data) {
            return this.ajaxCall(
                {
                    url: `${domain}${api}/elecseal/getPdfHashByGrantCode`,
                    data,
                },
                "post"
            );
        ...
        apiSignTask(data) {
            return this.ajaxCall(
                {
                    url: `${domain}${api}/elecseal/signTask`,
                    data,
                },
                "post"
            );
        ...
        // 获取签名值
        apiPushSignResult(data) {
            return this.ajaxCall(
                {
                    url: `${domain}${api}/elecseal/pushSignResult`,
                    data,
                },
                "post"
            );
        ...
        apiDetachedPdf(data) {
            return this.ajaxCall(
                {
                    url: `${domain}${api}/elecseal/detachedPdf`,
                    data,
                },
                "post"
            );
