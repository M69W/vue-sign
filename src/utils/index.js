 ...
 * @param {Object} form  递归使用的formData对象
 * @param {String} namespace formData的命名空间
 */
 ...
 * @param {Number} val - 值，必传
 */
export const checkIntegralAndZero = (val) => {
    var integralReg = /^(0|\+?[1-9][0-9]*)$/;
    return integralReg.test(val);
};
 ...
 * data, //数据
   ...
   param,//加密字段
   key,//加密字段
 * @param {Object} params  dataOBJ type
 */
export const encryption = (params) => {
    let { data, type, param, key = "iyin123456123456" } = params;
...
export const decrypt = (params) => {
    ...
    if (type === "Base64") {
        ...
        param.forEach((ele) => {
            ...
            result[ele] = CryptoJS.enc.Utf8.stringify(decrypt).toString();
        });
        return result;
    }
};
 ...
 *
 */
export const windowOnBeforeunloadFn = (e, cb) => {
    // e = e || window.event;
    ...
    // if ((b && e.clientY < 0) || e.altKey || e.ctrlKey) {
    //     console.log("zx");
